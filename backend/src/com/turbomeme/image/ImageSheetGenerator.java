...
public final class ImageSheetGenerator implements Constants
{
	...
	{
		final File jsonFile = new File(Paths.BACKEND_RESOURCE_DIR +
				BACKEND_IMAGE_ORDER_JSON_FILE_NAME);
		final JSONParser parser = new JSONParser();

		try
		{
			final MemeBank bank = ImageDataGenerator.loadMemeBank(
					Paths.BACKEND_RESOURCE_DIR + BACKEND_IMAGE_DATA_JSON_FILE_NAME);
			...
			double temp = (double) bank.getIdToMemeMap().size() *
					(double) IMAGE_SHEET_ICON_WIDTH / (double) IMAGE_SHEET_WIDTH;
			temp = Math.ceil(temp);
			int height = (int) (temp);
			height = height * IMAGE_SHEET_ICON_HEIGHT;
			final BufferedImage imageSheet = new BufferedImage(IMAGE_SHEET_WIDTH,
			...
			for (int i=0; i < imageOrderJson.size(); i++)
			{
				...
				graphics2D.drawImage(img, x, y, IMAGE_SHEET_ICON_WIDTH,
						IMAGE_SHEET_ICON_HEIGHT, null);
			...
			ImageIO.write(imageSheet, "jpg", new File(Paths.BACKEND_RESOURCE_DIR
					+ BACKEND_IMAGE_SHEET_FILE_NAME));
			ImageIO.write(imageSheet, "jpg", new File(Paths.UI_DIR +
					FRONTEND_IMAGE_SHEET_FILE_NAME));

			try
			{
				// Write backend file
				FileWriter file = new FileWriter(Paths.BACKEND_RESOURCE_DIR +
						BACKEND_IMAGE_MAP_FILE_NAME);
				...
				// Write frontend file
				file = new FileWriter(Paths.FRONTEND_DIR +
						FRONTEND_IMAGE_MAP_FILE_NAME);
